...
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #looking for something in this entire vector

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) #152 and 304 are day numbers
```
...
NTL.phys.data.1999<- 
  filter(NTL.phys.data,year4 %in% c(1999)) #could have also written year4 == 1999
...
> Answer: We don't filter using row number because rows can change data especially if you alter the dataset. So it is easier and more accurate to use the column in specific.
...
NTL.phys.data.T1990<-
  NTL.phys.data %>%
  filter(lakename %in% c("Tuesday Lake") & year4 %in% c(1990:1999) & month(sampledate)==7)
