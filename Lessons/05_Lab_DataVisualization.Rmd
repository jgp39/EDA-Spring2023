...
#can also add aesthetics in the ggplot first line
#Print the plot
print(Plot_text)
...
#here is just a better way to do the getwd. Can keep adding paths to it. Ex: data, processed
EPAair <- read.csv(
...
ggplot(PeterPaul.chem.nutrients, aes(x=month))+ 
  geom_bar() 
#don't want to do histogram because u want to graph discrete values

#Convert month to a factor -- with 12 levels, labelled with month names
factor(PeterPaul.chem.nutrients$month) #gives u actual values that are in the columns  NOT A GRAPH, all factors have levels. Levels are set of unique values

#Try again: Plot how many Lake observations occur each month
ggplot(PeterPaul.chem.nutrients, aes(x=factor(month)))+ 
  geom_bar() 
#doesn't know that 2 is 2 units from 5, weird looking x-axis

#Alter to include missing months
factor(PeterPaul.chem.nutrients$month,
       levels = 1:12,
       labels = month.abb)#changed factors from numbers to month names
ggplot(PeterPaul.chem.nutrients, aes(x=factor(month,levels = 1:12, labels = month.abb)))+ 
  geom_bar()+ 
  scale_x_discrete(drop= FALSE) #need to add this line so that the added levels will show up in the graph

#Tidy up the code
ggplot(PeterPaul.chem.nutrients, aes(x=factor(month,levels = 1:12, labels = month.abb)))+ 
  geom_bar()+ 
  scale_x_discrete(drop= FALSE)+
  labs(x="Month", y="Count") #labs adds labels
