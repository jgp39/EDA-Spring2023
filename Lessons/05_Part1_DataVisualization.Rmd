...
#adding the parameters at the top will ensure that it holds true for any plot created in this chunk
# Scatterplot 
...
print(O3plot) #x axis is date, y axis is ozone, will show up as a point

# Fix this code
O3plot2 <- ggplot(EPAair) + #provide dataframe you are working in
  geom_point(aes(x = Date, y = Ozone, color = "blue")) #added a color in the aesthetics is like adding another variable 
#correct version:
...
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) + #shape is assigning a different shape based on the year
  #color is a function of site name HOW TO COLOR CODE
  geom_point()
print(PMplot)
...
PMplot.faceted <-
  ...
  geom_point() + #want variables as points
