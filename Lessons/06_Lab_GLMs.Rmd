...
```{r, fig.height=3, fig.width=4, warning=TRUE}
#Exercise 1:
ozone.density<- ggplot(EPAair, aes(x=Ozone, fill= as.factor(Year)))+
  geom_density(alpha=0.5)+#alpha changes transparency
  scale_fill_manual(values = c("darkgreen","darkblue"))+
  labs(fill="",x="Ozone AQI values", y="Density")#adding empty "" means you add nothing
...
ex_02<- lm(data = EPAair, PM2.5~Ozone)#y~x is y (dependent variable) is a function of x (independent variable)
summary(ex_02) 
# lm runs a linear regression, does not spit out a plot
...
ozone.sitename<- EPAair%>%
  select(Month,PM2.5, Ozone)%>% #needed to use only numerical variables, you cannot use site name
  na.omit() #only omits na in a specific column drop.na will omit na's for all columns

ozone.corr <- cor(ozone.sitename)
corrplot(ozone.corr, method = "ellipse")#different plot than mixed
corrplot.mixed(ozone.corr, upper = "ellipse") #puts the labels inside the graph
...
ozonebyall.regression<- lm(data= ozone.sitename, PM2.5~Ozone+ Year+Month+ mean_lng)
