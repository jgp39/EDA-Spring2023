...
huc8_sf<- st_read('./Data/Spatial/NCHUC8.shp')
upper_neuse_sf<- huc8_sf %>% 
  filter(SUBBASIN== "Upper Neuse")

#Create a map
mapView(huc8_sf, col.regions= "orange")+
mapView(upper_neuse_sf, col.regions= "purple")

#create map in ggplot
ggplot()+
  geom_sf(data = huc8_sf, fill= 'purple', color= 'gray')+ #fill will fill the inside, color only does the border
  geom_sf(data = upper_neuse_sf, fill= 'orange')

#adding the plus gets them on top of each other
```
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  colClasses = c('HUC_12' = 'factor')) %>%  #makes sure that it reads this column values in as a factor and not a number
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #substr= substring, tells u to start at 1 and co to 8 and make it a new column
  group_by(HUC_8) %>% 
...
huc8_sf_join = merge(x = huc8_sf,
                     y = wind_raw,
                     by.x = 'HUC_8',
                     by.y = 'HUC_8') #encatchments shown as wind energy
